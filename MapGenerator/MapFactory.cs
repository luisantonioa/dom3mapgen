...
namespace MapGenerator
{
    public enum  DisplayColors
    {
      ...
      Taiga = 0x99aa77,
      Shrubland = 0x889977,
      TemperateDesert = 0xc9d29b,
      TemperateRainForest = 0x448855,
      TemperateDeciduousForest = 0x679459,
      Grassland = 0x88aa55,
      SubtropicalDesert = 0xd2b98b,
      TropicalRainForest = 0x337755,
      TropicalSeasonalForest = 0x559944
    };

    public enum ElevationGradientColors
    {
      Ocean = 0x008800,
      GradientLow = 0x008800,
      GradientHigh = 0xffff00
    };

    public enum MoistureGradientColors
    {
      Ocean = 0x4466ff,
      GradientLow = 0xbbaa33,
      GradientHigh = 0x4466ff
    };
    ...
    public class MapFactory
    {
        ...
        static public string islandSeedInitial = "85882-1";
    
        // GUI for controlling the map generation and view
        public Sprite controls = new Sprite();
        ...
        public Bitmap noiseLayer;// = new Bitmap(SIZE, SIZE);
    ...
    private static string[] _biomeMap =
    ...
    private void drawPathForwards(graphics:Graphics, path:Vector.<Point>) {
      for (var i:int = 0; i < path.length; i++) {
        graphics.lineTo(path[i].x, path[i].y);
      }
    }
    private void drawPathBackwards(graphics:Graphics, path:Vector.<Point>) {
    ...
    private uint interpolateColor(uint color0, uint color1, float f) 
    {
      var r = (uint)((1-f)*(color0 >> 16) + f*(color1 >> 16));
      var g = (uint)((1-f)*((color0 >> 8) & 0xff) + f*((color1 >> 8) & 0xff));
      var b = (uint)((1-f)*(color0 & 0xff) + f*(color1 & 0xff));
      if (r > 255) r = 255;
      if (g > 255) g = 255;
      if (b > 255) b = 255;
      return (r << 16) | (g << 8) | b;
    ...
    private void drawGradientTriangle(Graphics graphics, Vector v1, Vector v2, Vector v3, uint[] colors, Action fillFunction) {
      var m = new Matrix();
      ...
      var G = new Vector3D(-N.x/N.z, -N.y/N.z, 0);

      // Center of the color gradient
      var C = new Vector3D(V.x - G.x*((V.z-0.5)/G.length/G.length), V.y - G.y*((V.z-0.5)/G.length/G.length));
